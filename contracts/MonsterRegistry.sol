...
contract MonsterRegistry is Ownable {
    ...
    /* function tokenURI(AttributeData[] memory attrData)
        public
        view
        returns (string memory)
    {
        ...
        for (uint256 j = 0; j <= 90; j++) {
            ...
            if (i < len) {
                uint256 id = attrData[j].attrID;
                string memory value = attrMetadata[id];
                ...
                if (i == 1) {
                    data = abi.encodePacked(
                        ...
                        abi.encodePacked(solid.p2, y, solid.p3),
                        abi.encodePacked(
                            solid.value,
                            solid.p1,
                            toString(40 + xs * 100)
                        ),
                        abi.encodePacked(solid.p2, y, solid.p4),
                        abi.encodePacked(value, solid.p1)
                    );
                } else {
                    data = abi.encodePacked(
                        data,
                        abi.encodePacked(toString(6 + xs * 100), solid.p2, y),
                        abi.encodePacked(
                            solid.p5,
                            toString(id),
                            solid.p1
                        ),
                        abi.encodePacked(toString(24 + xs * 100), solid.p2, y),
                        abi.encodePacked(solid.p3, solid.value, solid.p1),
                        abi.encodePacked(toString(40 + xs * 100), solid.p2, y),
                        abi.encodePacked(solid.p4, value, solid.p1)
                    );
                }
            } else {
                if (i == 90) {
                    data = abi.encodePacked(
                        data,
                        abi.encodePacked(toString(6 + xs * 100), solid.p2, y),
                        abi.encodePacked(solid.p5, solid.question, solid.p1),
                        abi.encodePacked(toString(24 + xs * 100), solid.p2, y),
                        abi.encodePacked(solid.p3, solid.value, solid.p1),
                        abi.encodePacked(toString(40 + xs * 100), solid.p2, y),
                        abi.encodePacked(solid.p4, solid.discovered)
                    );
                } else {
                    data = abi.encodePacked(
                        data,
                        ...
                        abi.encodePacked(solid.p3, solid.value, solid.p1),
                        abi.encodePacked(toString(40 + xs * 100), solid.p2, y),
                        abi.encodePacked(solid.p4, solid.discovered, solid.p1)
                    );
                }
            }
        }
